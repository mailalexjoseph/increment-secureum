...
contract MinterRedeemer is Settlement, vAMM {
    ...
    event sellQuoteLong(uint256 quoteLong, address indexed user, address indexed reserve);
    event sellQuoteShort(uint256 quoteshort, address indexed user, address indexed reserve);

    /************************* functions *************************/

    /// @notice Check if user leverage allows operation
    function _leverageIsFine(address account, uint256 _amount) internal view returns (bool) {
