...
contract Reserve is Getter {
    ...
    function deposit(uint256 _amount, address _token) public {
        if (isAaveToken[_token]) {
            ...
            uint256 decimalsScaled = (scaledAmount * 10**18) / 10**IERC20Metadata(_token).decimals();
            balances[msg.sender].userReserve[_token] += decimalsScaled;
        } else {
            SafeERC20.safeTransferFrom(IERC20(_token), msg.sender, address(this), _amount);

            uint256 decimalsScaled = (_amount * 10**18) / 10**IERC20Metadata(_token).decimals();
    ...
    function allowWithdrawal(
        ...
        uint256 newMarginRatio = _marginRatio(newPortfolioValue, getUnrealizedPnL(account), getUserNotional(account));
    ...
    function withdraw(uint256 _amount, address _token) public {
        ...
        require(allowWithdrawal(msg.sender, _token, _amount), "Withdrawal would result in Liquidation");
        balances[msg.sender].userReserve[_token] -= _amount;
        if (isAaveToken[_token]) {
            uint256 unscaledAmount = balanceToScaledBalance(_amount, _token);
            uint256 decimalsUnscaled = (unscaledAmount * 10**IERC20Metadata(_token).decimals()) / 10**18;
            SafeERC20.safeTransfer(IERC20(_token), msg.sender, decimalsUnscaled);
        } else {
            uint256 decimalsUnscaled = (_amount * 10**IERC20Metadata(_token).decimals()) / 10**18;
            SafeERC20.safeTransfer(IERC20(_token), msg.sender, decimalsUnscaled);
        }
