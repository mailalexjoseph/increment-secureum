...
contract Getter is Storage {
    ...
    function getAssetPrice(address _oracleAddress) public view returns (uint256) {
        (, int256 price, , uint256 timeStamp, ) = AggregatorV3Interface(_oracleAddress).latestRoundData();
    ...
    function getAssetPriceByTokenAddress(address _tokenAddress) public view returns (uint256) {
    ...
    function getEntryPrice(address account) public view returns (uint256) {
        return (getUserNotional(account) * (10**18)) / ((getLongBalance(account) + getShortBalance(account)));
    }

    /// @notice Returns user balance of a given reserve tokens
    /// @param account user address
    /// @param _token token address
    function getReserveBalance(address account, address _token) public view returns (uint256) {
    ...
    function getAssetValue(address account, address token) public view returns (uint256) {
        address oracleAddress = getAssetOracle(token);

        uint256 tokenValue;
        if (isAaveToken[token]) {
            tokenValue =
                (scaledBalanceToBalance(getReserveBalance(account, token), token) * getAssetPrice(oracleAddress)) /
                (10**8);
        } else {
            tokenValue = (getReserveBalance(account, token) * getAssetPrice(oracleAddress)) / (10**8);
        }
    ...
    function getUnrealizedPnL(address account) public view returns (uint256) {
        ...
        uint256 unrealizedPnL = simplifiedSellAmount - notionalAmount;
    ...
    function getUserMarginRatio(address account) public view returns (uint256) {
        return _marginRatio(getPortfolioValue(account), getUnrealizedPnL(account), getUserNotional(account));
    }
    ...
    function _marginRatio(
        ...
        if (notionalValue > 0) {
            marginRatio = ((margin + unrealizedPnL) * 10**18) / notionalValue;
        }
        return marginRatio;
    }

    /************************* Aave helpers *************************/
    function scaledBalanceToBalance(uint256 amount, address token) internal view returns (uint256) {
        ILendingPool lendingpool = ILendingPool(lendingPoolAddressesProvider.getLendingPool());
        uint256 currentIndex = lendingpool.getReserveNormalizedIncome(aaveReserve[token]);
        return (amount * currentIndex) / (10**27);
    }

    function balanceToScaledBalance(uint256 amount, address token) internal view returns (uint256) {
        ILendingPool lendingpool = ILendingPool(lendingPoolAddressesProvider.getLendingPool());
        uint256 currentIndex = lendingpool.getReserveNormalizedIncome(aaveReserve[token]);
    ...
    function _convertDollarToAssets(uint256 _amount, address _redeemAsset) internal view returns (uint256) {
