...
describe('Perpetual', () => {
  ...
  describe('Opening/closing long/short trading positions', () => {
    ...
    it('Profit (or loss) should be reflected in the user balance in the Vault', async () => {
      ...
      const aliceVaultBalanceBeforeOpeningPosition =
        await alice.vault.getBalance(alice.address);
    ...
    it('No exchange rate applied when opening and closing LONG positions', async () => {
      ...
      aliceTraderPosition = await alice.perpetual.getUserPosition(
        alice.address
      );
      ...
      expect(vQuoteLiquidityAfterPositionCreated.eq(expectedAdditionalVQuote))
        .to.be.true;
    ...
    it('EUR_USD exchange rate applied when opening and closing SHORT position', async () => {
      ...
      // set-up
      await setUpPoolLiquidity(bob, depositAmount);
      await alice.perpetual.deposit(
        alicePositionNominalSize,
        alice.usdc.address
      );
