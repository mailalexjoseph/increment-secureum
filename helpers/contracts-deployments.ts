import {
  ...
  CurveCryptoContractsConstructorArgumentsSeparate,
  CurveCryptoContractsConstructorArgumentsFactory,
  tEthereumAddress,
} from '../helpers/types';
import {getReserveAddress, getFeedRegistryAddress} from './contracts-getters';
...
export function getCryptoSwapConstructorArgsSeparate(
  ...
  const cryptoSwapConstructorArgs: CurveCryptoContractsConstructorArgumentsSeparate =
    {
      owner: deployer,
...
export function getCryptoSwapConstructorArgs(
  ...
  const cryptoSwapConstructorArgs: CurveCryptoContractsConstructorArgumentsFactory =
    {
      _name: _name,
      _symbol: _symbol,
      _coins: [quoteToken, baseToken],
      A: BigNumber.from(5000)
        .mul(2 ** 2)
        .mul(10000),
      gamma: utils.parseEther('0.0001'),
